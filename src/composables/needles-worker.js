...
class Block {
  ...
  get bins() {
    ...
    this._bins = Array(this.channelCount)
      .fill(null)
      .map(() => this.createBin());
    return this._bins;
  }
...
class LoudnessMeter {
  ...
  get blockMarginCounter() {
    return (this._blockMarginCounter =
      this._blockMarginCounter || new Counter(this.blockMarginLengthInSamples));
  }

  get updateCounter() {
    return (this._updateCounter =
      this._updateCounter ||
      new Counter(this.updateLengthInSamples, this.update, this));
  }
  ...
  input(input) {
    ...
    if (
      !this.blocks.length ||
      this.blockMarginCounter.willMeetTarget(sampleCount)
    ) {
